...
official LongBench ones -- the same code path the evaluation used.
    ...
    python tools/verify_samples.py --task qasper --verbose
"""
...
from tools.export_per_document import evaluators  # noqa: E402
...
def main() -> None:
    ...
    if not files:
        raise SystemExit(f"no samples under {SAMPLES}; run tools/export_samples.py")
