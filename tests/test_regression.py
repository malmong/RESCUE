...
Every number in the paper comes out of ``rescue/eviction.py``, so a change
...
selection because the comparison is bit-exact rather than statistical.
    ...
    python tests/test_regression.py --save baseline.json    # before editing
    python tests/test_regression.py --check baseline.json   # after editing
...
def install_recorder(record) -> list:
    ...
    """
    from rescue import eviction as E
...
def fingerprint(policy: str) -> dict:
    import torch
    from rescue.eviction import DenseEvictionGenerator
