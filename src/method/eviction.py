...
from src.utils.rkv import rkv_final_score
from src.method import LearnedScorer
...
def _call_with_query_capture(model, fn):
    ...
    src.method.future_contrib.extract.capture_sequence uses for training -- so
...
class DynamicEvictionState:
    def __init__(self, cfg: BaselineConfig, model, learned_scorer: LearnedScorer | None = None):
        ...
        # state for rfc_objective=="H" (src.method.future_contrib.active_pattern) --
        ...
        if policy == "rfc":
            ...
            if self.cfg.rfc_objective == "H" and attentions:
                ...
                for layer_idx, layer_attn in enumerate(attentions):
                    ...
                    # needs (see src.method.future_contrib.active_pattern).
    ...
    def _h2o_keep(self, layer_idx: int, budget: int) -> torch.Tensor:
        ...
        from src.utils.scoring import select_topk_indices
...
class RaggedRKVState:
    ...
    src/baselines/rkv.py's module docstring for the full score formula.
    ...
    reduction happens inside src.utils.rkv via MAX, matching the
...
class DenseEvictionGenerator:
        ...
        if cfg.normalized_policy() in {"kvp", "foresightkv", "lookaheadkv", "rfc"}:
            ...
            if cfg.normalized_policy() == "kvp":
                ...
                if cfg.rfc_objective == "oracle":
                    ...
                    # future importance -- see src.method.scorer
    ...
    def generate_one(self, prompt: str, max_new_tokens: int, max_seq_len: int) -> str:
        ...
        if _probe_attn is not None and _probe_recent_w != 0.0:
            from src.utils.scoring import set_probe_rows
        ...
        if _probe_attn is not None and _probe_recent_w != 0.0:
            from src.utils.scoring import clear_probe_rows
            clear_probe_rows()
        if _tm:
