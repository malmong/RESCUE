...
class BaselineConfig:
    ...
    rfc_lambda_per_layer: tuple[float, ...] | None = None  # overrides rfc_lambda per-layer when set (layer-wise gate experiment found reuse-predictor quality varies a lot by layer)
    rfc_objective: str = "legacy"  # "legacy" (Q/K FutureQueryPredictor) | "A"/"B"/"C"/"D"/"G" (src/method/future_contrib/train_unified.py checkpoints) | "E" (hybrid: legacy reuse x objective-C impact)
    rfc_impact_checkpoint: str | None = None  # objective "E" only: path to an objective-C train_unified.py checkpoint supplying the impact head
    ...
    rfc_recent_style: str = "contribution"  # "contribution" (default: Gram-matrix true output-contribution, src.method.future_contrib.contribution.recent_contribution_from_attn) | "laprox" (LaProx's own exact ||attn||_2 * ||V W_O|| formula + its snapkv_obs_window protected-window convention -- lambda=0 + rfc_allocation="global_layer" + this reproduces LaProx's own score exactly, a verification baseline)
    ...
    rfc_eval_horizon_idx: int | None = None  # rfc_objective="MH" only: which trained horizon's prototype to serve at eval (default 0 = shortest, see src.method.rfc_multi.MultiHeadRFCScorer)
    rfc_eval_blend: bool = False  # rfc_objective="MH" only: blend all trained horizons' prototypes (short-weighted) instead of serving a single one
    rfc_layer_head_gain_path: str | None = None  # path to a src/method/future_contrib/global_gain_train.py checkpoint ([num_layers, num_kv_heads] tensor) -- replaces the single scalar rfc_lambda with a per-(layer, kv_head) value
    future_fusion: str = "fixed"  # rfc_objective="MP" only: "fixed" (default, unchanged -- lambda=rfc_lambda) | "concentration" (lambda=lambda_base*(1+confidence_alpha*Q)) | "agreement" (lambda=lambda_base*(1+confidence_alpha*A)) | "concentration_agreement" (lambda=lambda_base*(1+confidence_alpha*Q*A)), Q=sum_i F_i^2 (raw, unnormalized prototype-mixture concentration), A=exp(-agreement_beta*D_bar), D_bar=mean prototype disagreement Std_k(P_i^(k)). No head-wise/layer-wise score normalization anywhere in this path -- see src.method.eviction._adaptive_lambda_from_confidence.
    lambda_base: float = 2.0  # future_fusion != "fixed" only: base lambda scaled by (1 + confidence_alpha*confidence)
    confidence_alpha: float = 1.0  # future_fusion != "fixed" only: scales how much the confidence signal can boost lambda above lambda_base
    agreement_beta: float = 1.0  # future_fusion in {"agreement","concentration_agreement"} only: decay rate converting raw prototype disagreement D_bar into agreement A=exp(-agreement_beta*D_bar)
    future_confidence_log_path: str | None = None  # if set, appends one JSONL record per (layer, eviction-step) with raw future/observed score stats, Q, D_bar, A, C, and the lambda actually used -- rfc_objective="MP" only
    rfc_adaptive_lambda: bool = False   # derive lambda per layer from the correction's own concentration
    rfc_adaptive_a: float = 18.0
    rfc_adaptive_b: float = 55.0
    rfc_adaptive_max: float = 8.0
    rfc_adaptive_scope: str = "layer"   # "layer" = per-layer lambda, "doc" = one per document
    rfc_adaptive_warmup: int = 4        # doc scope: layers sampled before freezing
    ...
    rfc_ppl_lambda_slope: float = 0.0   # >0 replaces the step gate with lambda = clamp(slope*(entropy-tau), 0, rfc_ppl_lambda_max). Step gate is the slope->inf limit; fixed lambda is the tau->-inf limit.
    rfc_ppl_lambda_max: float = 0.0     # cap for the continuous form; 0 = 4x --rfc-lambda
    ...
    rfc_gate_scope: str = "doc"      # "doc" = one gate decision per document (frozen after rfc_adaptive_warmup layers); "layer" = every layer decides for itself from its OWN overlap. Online measurement showed overlap is a LAYER property (layers differ ~20x within a document while the profile barely moves between tasks), so "doc" cannot separate tasks and "layer" is what the data supports.
    rfc_lambda_gate_tau: float = 0.0   # >0 enables the REDUNDANCY GATE: per document, measure how much the correction's own top-B set overlaps the recent-window score's top-B set; if the median over the first rfc_adaptive_warmup layers is BELOW tau the correction is carrying information the base ranker does not have, so lambda stays at --rfc-lambda -- otherwise the correction is largely restating the base ranking and lambda is set to EXACTLY 0, which with rfc_combine_mode="scale_additive" reduces to the recent-window score untouched (i.e. the baseline, never a loss). 0.0 = gate disabled.
    ...
    rkv_retain_direction: str = "last"  # official default; "first"|"last_percent"|"first_percent" also supported (see src.utils.rkv)
